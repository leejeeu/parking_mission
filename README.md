...
자율주행 주차 미션: Nav2(ROS2 Navigation2) 기반으로 출발 지점에서 지정된 주차영역(A, B)을
순서대로 방문해 정밀 주차한 뒤 출발지로 복귀하는 패키지. 실차(xycar, 애커먼만/자전거 모델
...
## 3. 미션 시퀀스 — 출발 → A 주차 → B 주차 → 출발지 복귀

> **[2026-08-24 정정]** 이 절은 원래 "A/B 자동 판단 없음"이라는 제목으로 "A 또는 B 둘 중
> 하나를 사람이 골라 그쪽에만 간다"는 걸 한계로 적어뒀었다. 대회 측이 제공한 공식 맵
> 안내문("출발지점 출발 → A구역 주차 → B구역 이동 → B구역 주차 → 출발지점 복귀")을
> 확인한 결과, 이건 "A/B 선택"의 문제가 아니라 **A와 B를 순서대로 둘 다 방문하고 출발지로
> 복귀까지 해야 하는** 훨씬 큰 범위의 미션이었다 — 정정 전 구현(한 구역만 가고 끝)은
> 실제 미션 범위를 놓치고 있었다.

`parking_navigator.py`는 이제 `MISSION_LEGS = ['A', 'B', 'START']` 순서를 고정 시퀀스로
수행한다(레그마다 Nav2 목표 전송 → 필요하면 병목구간 우회 → 정밀 접근(도킹) 순). A/B
좌표는 여전히 `zone_a_x/y/yaw`, `zone_b_x/y/yaw` 파라미터에 고정값(A=(0.0, 4.2, 0°),
B=(2.1, 3.3, -90°))으로 있고, 출발지 복귀 목표는 기존 `start_x/y/yaw` 파라미터를 그대로
재사용한다.

`parking_zone` launch 인자 기본값은 `'FULL'`(전체 시퀀스)이며, `'A'`/`'B'`/`'START'`처럼
개별 레그 이름을 주면 그 레그 하나만 실행한다(디버깅/개별 구간 튜닝용, 과거 단일 레그
동작과 동일).

레그 하나가 타임아웃 등으로 실패해도 미션은 멈추지 않고 다음 레그로 계속 진행한다 —
경기 규정상 한 구역을 놓쳤다고 나머지(다른 구역, 복귀)까지 포기하면 손해이므로, 부분
점수라도 확보하는 쪽을 기본 동작으로 삼았다.

카메라/AR태그 등으로 "어느 자리가 비어있는지" 실시간으로 감지해서 A/B 순서를 동적으로
바꾸는 로직은 없다 — 공식 안내문이 순서(A 먼저, B 다음)를 명시하고 있어 필요하지 않다고
판단했다. 만약 실제 대회에서 순서가 매 라운드 달라질 수 있다면 `parking_zone` 대신 별도
파라미터로 순서를 받는 확장이 필요하다.
