...
자율주행 주차 미션: Nav2(ROS2 Navigation2) 기반으로 출발 지점에서 지정된 주차영역(A 또는 B)까지
스스로 이동해 정밀 주차하는 패키지. 실차(xycar, 애커먼만/자전거 모델 조향)와 Gazebo 시뮬레이션을
동일한 코드로 구동한다.
...
## 3. 주차영역(A/B) 선택 — ⚠️ 현재는 자동 판단이 없다

`parking_navigator.py`는 A/B 좌표를 **코드에 미리 박아둔 고정값**으로만 갖고 있고
(`zone_a_x/y/yaw`, `zone_b_x/y/yaw` 파라미터, 기본값 A=(0.0, 4.2, 0°), B=(2.1, 3.3, -90°)),
어느 쪽으로 갈지는 `parking_zone` launch 인자(기본값 `'A'`)로 **사람이 직접 지정**한다.

카메라/AR태그 등으로 "어느 자리가 비어있는지" 실시간으로 감지해서 자동으로 A/B를 고르는 로직은
이 저장소 어디에도 없다(`parking_navigator.py`, `parking_mission.launch.py` 전체 검색 확인,
2026-08-19). 실제 대회 룰이 "출발 직전 A/B가 랜덤으로 주어짐"이라면, 그 값을 무엇으로 어떻게
알려받는지(예: 별도 신호/토픽, 심판이 launch 인자로 직접 지정, 카메라로 표지판 인식 등)에 따라
감지 로직을 추가로 구현해야 한다 — 지금은 완전히 빠져 있는 부분이니 실차 투입 전에 반드시
확인/보완 필요.
...
- **A/B 자동 판단 없음** — 위 3번 항목 참고. 실차 투입 전 확인 필요.
